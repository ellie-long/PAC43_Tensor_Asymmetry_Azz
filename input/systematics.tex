...
under 6\% \cite{keller2,keller3}. The figure of merit (FOM) for a tensor polarized solid state target can be defined as,
\begin{equation}
FOM=n_tf^2P_{zz}^2
\end{equation}
...
The only documented instance with ammonia polarized targets and CEBAF $\sim$100~nA beams of a possible rearrangement of material about the target NMR coil that
...
%In addition, as long as the LHe is superfluid ($<2$ K), its flow can not lead to material rearrangements.  The LHe that is fed at the bottom of the nose piece coming from the separator is below 2~K, so emptying  and refilling does not have any effect.
